...
import { OrganizationSchema, WebSiteSchema, ProductSchema } from '../components/StructuredData';

const Home: React.FC = () => {
  return (
    <div className="relative">
      <SEO
        title="Aadhava Attan | Buy Authentic South Indian Snacks & Traditional Sweets Online"
        description="Order authentic handmade South Indian snacks online. Traditional Murukku, Madras Mixture, and more - made with no palm oil, no preservatives. Worldwide shipping to India, Singapore, and Canada."
        canonicalUrl="/"
      />
      <OrganizationSchema />
      <WebSiteSchema />
      ...
      <section className="relative w-full bg-transparent px-0 overflow-hidden flex flex-col justify-center items-center pt-16 pb-16">
        ...
        {/* SEO-friendly H1 (visually styled to match design) */}
        <h1 className="sr-only">Aadhava Attan - Authentic South Indian Snacks & Traditional Sweets</h1>
          ...
          {PRODUCTS.map((product, index) => (
            <AnimatedDiv key={product.id} delay={index * 0.1}>
              <Link to={`/product/${product.slug}`} className="block h-full">
                <div className="group relative bg-transparent rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 border-4 border-[#7A0F12] h-full cursor-pointer hover:-translate-y-2">
                  ...
                  <div className="p-8 flex flex-col items-center text-center h-full justify-between bg-[url('https://www.transparenttextures.com/patterns/white-diamond.png')]">
                    <div className="w-48 h-48 rounded-full bg-[#7A0F12] border-4 border-[#D7A846] flex items-center justify-center mb-6 group-hover:rotate-12 transition-transform duration-300 overflow-hidden relative shadow-inner">
                      <img src={product.image} alt={product.imageAlt} loading="lazy" className="w-full h-full object-cover transform scale-110 group-hover:scale-125 transition-transform duration-700" />
                    </div>
